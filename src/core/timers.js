/**
 * Servicio global de temporizadores.
 *
 * Vive fuera de las herramientas: el temporizador, el pomodoro y el
 * cronometro siguen corriendo aunque la vista se desmonte, aunque estes en
 * otra pantalla y aunque recargues la pagina. La herramienta y la minibarra
 * son solo vistas suscritas a este servicio.
 *
 * El reloj se calcula siempre contra la hora real (`Date.now()`), nunca
 * sumando ticks: asi no se desfasa si el navegador ralentiza la pestana.
 */
...
import { alarmSound } from '../tools/temporizador/alarms.js';
import { t } from './i18n.js';
import { transcurrido, restante, vueltasConParcial as parciales, formatClock as reloj } from '../lib/stopwatch.js';

/** Avisa a todas las vistas suscritas. */
function avisar() {
  for (const fn of [...oyentes]) {
    try { fn(); } catch (err) { console.error('[temporizadores] vista', err); }
  }
}

/** Se suscribe una vista. Devuelve la funcion para darse de baja. */
...
export const PRESETS = [1, 3, 5, 10, 15, 20, 25, 45, 60];
...
const state = {
  ...
  round: 1,
  finished: false,
  iniciado: false          // se arranco y aun no se ha reiniciado: sigue "en curso"
};
...
const oyentes = new Set(); // vistas suscritas (herramienta, minibarra, widget)
let restored = false;
...
function restore() {
  ...
  state.iniciado = saved.iniciado === true;
...
const CRONO_KEY = 'cronometro';
export const MAX_VUELTAS = 99;

export const crono = {
  running: false,
  startedAt: 0,      // marca real del ultimo arranque
  acumulado: 0,      // milisegundos de antes de la ultima pausa
  vueltas: []        // marcas acumuladas, en milisegundos
};

let cronoRestaurado = false;

function guardarCrono() {
  storage.set(CRONO_KEY, {
    running: crono.running, startedAt: crono.startedAt,
    acumulado: crono.acumulado, vueltas: crono.vueltas
  });
}

/** Milisegundos transcurridos, contra la hora real. */
export function cronoMs() {
  return transcurrido(crono);
}

export function restoreCrono() {
  if (cronoRestaurado) return;
  cronoRestaurado = true;
  const saved = storage.get(CRONO_KEY, null);
  if (!saved || typeof saved !== 'object') return;
  crono.acumulado = Number.isFinite(saved.acumulado) && saved.acumulado >= 0 ? saved.acumulado : 0;
  crono.vueltas = Array.isArray(saved.vueltas)
    ? saved.vueltas.filter(Number.isFinite).slice(0, MAX_VUELTAS)
    : [];
  // Si estaba en marcha al cerrar, se sigue contando desde entonces: el
  // cronometro mide tiempo real, no tiempo de pantalla encendida.
  if (saved.running && Number.isFinite(saved.startedAt) && saved.startedAt <= Date.now()) {
    crono.running = true;
    crono.startedAt = saved.startedAt;
  }
}
...
export function cronoPause() {
  if (!crono.running) return;
  crono.acumulado = cronoMs();
  crono.running = false;
  guardarCrono();
  avisar();
}

export function cronoToggle() {
  crono.running ? cronoPause() : cronoStart();
}

export function cronoReset() {
  crono.running = false;
  crono.acumulado = 0;
  crono.startedAt = 0;
  crono.vueltas = [];
...
/** Marca una vuelta. Devuelve false si no corre o si ya no caben mas. */
export function cronoLap() {
  if (!crono.running || crono.vueltas.length >= MAX_VUELTAS) return false;
  crono.vueltas = [...crono.vueltas, cronoMs()];
  guardarCrono();
  avisar();
  return true;
}
...
export function vueltasConParcial() {
  return parciales(crono.vueltas);
}

/* ---------------- Vista de conjunto ---------------- */

/** Arranca el servicio al abrir la app, no al abrir la herramienta. */
...
export { state, leftMs, start, pause, reset, setDuration, switchMode,
  formatClock, restore, stopAlarm, listenToServiceWorker, phaseMinutes };

/**
 * Lo que hay en marcha ahora mismo, para la minibarra y los widgets.
 * `principal` es lo que termina antes; `otros` cuantos mas hay.
 */
export function activos() {
  const lista = [];
  if (state.running || ringing || state.iniciado) {
    lista.push({
      tipo: 'temporizador',
      claveNombre: state.mode === 'pomodoro' ? `temporizador.phase.${state.phase}` : 'temporizador.tab.countdown',
      ms: leftMs(),
      cuentaAtras: true,
  ...
  if (crono.running || cronoMs() > 0) {
    lista.push({
      tipo: 'cronometro',
      claveNombre: 'temporizador.tab.crono',
      ms: cronoMs(),
      cuentaAtras: false,
      running: crono.running,
      sonando: false,
      orden: Infinity   // no termina nunca: va detras del que si termina
    });
  }
  lista.sort((a, b) => a.orden - b.orden);
  return { principal: lista[0] || null, otros: Math.max(0, lista.length - 1) };
}

/** Pausa o reanuda lo que muestre la minibarra. */
export function toggle(tipo) {
  if (tipo === 'cronometro') { cronoToggle(); return; }
  state.running ? pause() : start();
}

/** Para y deja a cero lo que muestre la minibarra. */
export function detener(tipo) {
  if (tipo === 'cronometro') { cronoReset(); return; }
  reset();
}
